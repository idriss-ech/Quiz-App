...
const AddQuizForm: React.FC<AddQuizFormProps> = ({ onQuizChange, initialData }) => {
    ...
    const setCorrectChoice = (questionId: string, choiceId: string) => {
        const updatedQuestions = questions.map(q => q.id === questionId ? { ...q, correctChoiceId: choiceId } : q);
        setQuestions(updatedQuestions);
        notifyParent(title, description, updatedQuestions);
    };
        ...
        <IonContent className="ion-padding">
            <IonList>
                ...
                {questions.map((q, qIndex) => (
                    <div key={q.id} style={{ border: '1px solid #ccc', borderRadius: '8px', marginBottom: '16px', padding: '8px' }}>
                        <IonItem lines="none">
                            <IonInput
                            ...
                            />
                            <IonButton fill="clear" color="danger" slot="end" onClick={() => removeQuestion(q.id)}>
                                <IonIcon slot="icon-only" icon={trashOutline} />
                            </IonButton>
                        </IonItem>

                        <IonRadioGroup value={q.correctChoiceId} onIonChange={e => setCorrectChoice(q.id, e.detail.value)}>
                            <IonListHeader>
                                <IonLabel>Choices</IonLabel>
                                <IonButton fill="clear" size="small" onClick={() => addChoice(q.id)}>
                            ...
                            {q.choices.map((choice) => (
                                <IonItem key={choice.id}>
                                    ...
                                    <IonInput
                                        ...
                                        value={choice.text}
                                        onIonInput={e => updateChoiceText(q.id, choice.id, e.detail.value!)}
                                    />
                                    <IonButton fill="clear" color="danger" slot="end" onClick={() => removeChoice(q.id, choice.id)}>
                                        <IonIcon slot="icon-only" icon={trashOutline} />
                                    </IonButton>
                                </IonItem>
                            ))}
